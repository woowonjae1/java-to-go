...
import { ChapterIcon, chapterColorMap } from './chapter-icon'
    ...
    <header className="sticky top-0 z-40 w-full border-b border-card-border glass">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <nav className="hidden md:flex items-center gap-4">
          ...
          >
            ...
            {/* Dropdown panel container to bridge the hover gap */}
            {isDropdownOpen && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 pt-1.5 z-50">
                <div className="w-[520px] bg-card border border-card-border rounded-2xl shadow-xl p-4 grid grid-cols-2 gap-3 animate-in fade-in slide-in-from-top-1 duration-200">
                  {chapters.map((ch, idx) => {
      ...
      {mobileOpen && (
        <nav className="md:hidden border-t border-card-border bg-card p-4 space-y-1">
          {chapters.map((ch) => {
            const colors = chapterColorMap[ch.id] || { text: 'text-foreground' }
            return (
              <Link
                key={ch.id}
                href={`/chapters/${ch.id}`}
