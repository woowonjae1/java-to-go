...
export function CodeDuel({ title, javaCode, goCode, highlights = [], description, outline = [] }: CodeDuelProps) {
  ...
  const handleOutlineClick = (sec: OutlineSection, idx: number) => {
    ...
    setTimeout(() => {
      if (javaScrollRef.current) {
        const javaEl = javaScrollRef.current.querySelector(`.line-container-${sec.javaLine}`) as HTMLElement
        if (javaEl) {
      ...
      if (goScrollRef.current) {
        const goEl = goScrollRef.current.querySelector(`.line-container-${sec.goLine}`) as HTMLElement
        if (goEl) {
  ...
  const javaKeywords = highlights.map((h) => h.java)
  const goKeywords = highlights.map((h) => h.go)
    ...
    >
      ...
      <div className="px-5 py-4 border-b border-card-border flex items-center justify-between flex-wrap gap-3 bg-card-bg/50 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2 text-xs">
          {(javaLines.length > 18 || goLines.length > 18) && (
            <button
              onClick={() => setIsExpanded(!isExpanded)}
              className={`px-2.5 py-1 rounded-md border font-medium transition-all duration-200 flex items-center gap-1.5 cursor-pointer
                ${isExpanded
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-md border font-medium transition-all duration-200 flex items-center gap-1 cursor-pointer
              ${syncEnabled
                ? 'bg-accent-light border-accent/20 text-accent hover:bg-accent-light/80'
                : 'bg-muted border-card-border text-muted-fg hover:text-foreground hover:bg-muted-hover'}`}
            title="同步滚动双侧代码"
      ...
      <div className="flex divide-x divide-card-border bg-code-bg">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-card-border">
          {/* Java panel */}
          <div
            className={`relative flex flex-col ${activeMobileTab === 'java' ? 'block' : 'hidden lg:flex'}`}
            onMouseEnter={() => {
              activeScrollPanel.current = 'java'
            }}
          >
            {/* Header decor */}
            <div className="flex items-center justify-between px-4 py-2 bg-java-bg border-b border-java-border/30">
              <div className="flex items-center gap-1.5">
                <span className="text-base">☕</span>
                <span className="text-xs font-bold text-java">Java</span>
              </div>
              <span className="text-[10px] text-zinc-500 font-mono">source.java</span>
            </div>

            <div className="relative flex-1 flex overflow-hidden bg-code-bg">
              <CopyButton text={javaCode} />
              <div
                ref={javaScrollRef}
                onScroll={() => handleScroll('java')}
                className={`relative w-full overflow-auto p-4 flex flex-col font-mono text-sm leading-relaxed transition-[max-height] duration-500 ease-in-out ${
                  isExpanded ? 'max-h-[2500px]' : 'max-h-[480px]'
                }`}
              >
                {javaHtmlLines.map((lineHtml, i) => {
                  const lineNum = i + 1
                  const isFlashing = javaHighlightLine === lineNum
                  return (
                    <div
                      key={i}
                      className={`line-container-${lineNum} flex items-start py-0.5 px-2 -mx-2 rounded transition-all duration-300 relative z-10 ${
                        isFlashing ? 'bg-accent/20 border-l-4 border-accent pl-1' : ''
                      }`}
                    >
                      {/* Line Number */}
                      <div className="w-8 shrink-0 text-right text-zinc-600 pr-3 border-r border-zinc-850 select-none font-mono text-xs leading-[1.75rem]">
                        {lineNum}
                      </div>
                      {/* Code text */}
                      <pre className="flex-1 pl-3 font-mono text-sm text-zinc-300 whitespace-pre-wrap break-all overflow-hidden select-text leading-[1.75rem]">
              ...
              {!isExpanded && javaLines.length > 18 && (
                <div
                  onClick={() => setIsExpanded(true)}
                  className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[var(--code-bg)] via-[var(--code-bg)]/90 to-transparent cursor-pointer flex items-end justify-center pb-3 hover:via-[var(--code-bg)] transition-all z-20"
                  title="点击展开完整代码"
                >
                  <span className="text-[11px] font-semibold text-accent bg-accent-light/95 border border-accent/20 px-3 py-1.5 rounded-full shadow-md backdrop-blur-sm hover:scale-105 active:scale-95 transition-all">
                    展开完整代码 ({javaLines.length} 行) ↓
                  </span>
                </div>
              )}
            </div>
          ...
          {/* Go panel */}
          <div
            className={`relative flex flex-col ${activeMobileTab === 'go' ? 'block' : 'hidden lg:flex'}`}
            onMouseEnter={() => {
              activeScrollPanel.current = 'go'
            }}
          >
            {/* Header decor */}
            <div className="flex items-center justify-between px-4 py-2 bg-go-bg border-b border-go-border/30">
              <div className="flex items-center gap-1.5">
                <span className="text-base">🐹</span>
                <span className="text-xs font-bold text-go">Go</span>
              </div>
              <span className="text-[10px] text-zinc-500 font-mono">main.go</span>
            </div>

            <div className="relative flex-1 flex overflow-hidden bg-code-bg">
              <CopyButton text={goCode} />
              <div
                ref={goScrollRef}
                onScroll={() => handleScroll('go')}
                className={`relative w-full overflow-auto p-4 flex flex-col font-mono text-sm leading-relaxed transition-[max-height] duration-500 ease-in-out ${
                  isExpanded ? 'max-h-[2500px]' : 'max-h-[480px]'
                }`}
              >
                {goHtmlLines.map((lineHtml, i) => {
                  const lineNum = i + 1
                  const isFlashing = goHighlightLine === lineNum
                  return (
                    <div
                      key={i}
                      className={`line-container-${lineNum} flex items-start py-0.5 px-2 -mx-2 rounded transition-all duration-300 relative z-10 ${
      ...
      {isExpanded && (javaLines.length > 18 || goLines.length > 18) && (
      ...
      {highlights.length > 0 && (
        <div className="px-4 py-2.5 bg-accent-light text-[11px] text-accent border-t border-card-border
          ...
          <span className="hidden lg:block text-[10px] text-muted-fg font-mono">💡 支持滚轮同步上下对照</span>
        </div>
      )}
    </motion.div>
  )
